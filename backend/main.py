...
### 🔒 subagent 幾乎都要填 `output.expect`(防「做一半就 done」)

subagent 是 LLM 多輪、**會偷懶**:任務是「校正**全部**文字」「翻譯**整份**」「逐一處理**每一筆**」,
它常做幾項就 `done`。系統驗證階梯(已對齊 skill 節點):
- **有填 `output.expect`** → 跑**深度驗證**(驗證 agent 會讀產出檔、查完整性/遺漏)→ 做一半被打回重做。
- **沒填 `output.expect`** → 只有淺層 LLM 驗證、抓得到空殼、但**抓不到「做了一半」**。

**鐵律**:任務含「全部 / 所有 / 每一 / 逐一 / 完整 / 整份」這種**全量**語意,
subagent step **必填 `output.expect`**、把**完整性驗收條件**寫進去,例如:
- 「校正全部文字」→ `expect: 全文每段都已校正、與原文段落數一致、無任一段被跳過`
- 「翻譯整份文件」→ `expect: 原文每段都有對應譯文、無漏譯、段落數一致`
- 「逐一處理每筆評論」→ `expect: 輸入幾筆輸出就幾筆、無遺漏`

> expect 寫「可被檢查的數量/對應關係」(段落數一致、筆數相同)比寫「品質好」更能讓驗證抓出偷懶。
> 非全量的探索任務(研究、debug)也建議填 expect(至少觸發深度驗證);純試錯無明確產出才可省。
