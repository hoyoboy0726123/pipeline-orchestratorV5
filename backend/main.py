...
**關鍵:expect 是雙面刃,只給「完整=可客觀數」的任務,不要給研究/判斷型。**
...
🚫 **不要填 expect(研究 / 探索 / 評估 / 規劃 / debug)**:這類產出是**判斷性 prose、沒有客觀
「完整=多少」基準**。硬填 expect(例「要有明確來源連結、多面向、資料量足夠」)→ 深度驗證會把
**做得對但簡短/沒貼滿 URL** 的正常研究輸出**誤殺成 failed**(實測踩過)。研究型的品質改靠
**role 本身的系統提示**(researcher/report_writer 已內建禁腦補/要附來源的規範)把關,**不靠 expect**。
- ❌ researcher / trend_analyst / report_writer / competitor_analyst → **不要填 expect**
